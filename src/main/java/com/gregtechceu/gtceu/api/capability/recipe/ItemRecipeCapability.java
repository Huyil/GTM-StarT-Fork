...
public class ItemRecipeCapability extends RecipeCapability<Ingredient> {
    ...
    private static List<Object2LongMap<ItemStack>> getInputContents(IRecipeCapabilityHolder holder) {
        ...
        var invs = new ArrayList<Object2LongMap<ItemStack>>(handlerGroups.size() + 1);
    ...
    @SafeVarargs
    private static Object2LongOpenCustomHashMap<ItemStack> getRecipeHandlerInputContents(Stream<RecipeHandlerList>... streams) {
        var strat = ItemStackHashStrategy.comparingAllButCount();
        var inv = new Object2LongOpenCustomHashMap<>(strat);
            ...
            for (var handler : handlerList.getCapability(ItemRecipeCapability.CAP)) {
                for (var content : handler.getContents()) {
                    if (content instanceof ItemStack stack && !stack.isEmpty()) {
                        inv.addTo(stack, stack.getCount());
                    }
                }
            }
        });
